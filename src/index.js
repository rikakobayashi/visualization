...
const d3_sankey_1 = require("d3-sankey");
d3.json("data_test.json", function (error, data) {
    ...
    const a_line2 = [];
    const place3 = []; //nodeの中心位置を記録する{r_x,r_y}
    const a_line3 = [];
    const IR = data.Risk1;
    const SR = data.Risk2;
    ...
    const start_count = 0;
    const end_count = -1;
    const pre_start_count = 0;
    const pre_end_count = -1;
    const pre_parallel = 0;
    ...
    const sankey1 = d3_sankey_1.sankey()
        .nodeWidth(36)
        .nodePadding(40)
        .size([data.order_all.length * link_length + 110, height_s]);
    const sankey2 = d3_sankey_1.sankey()
        .nodeWidth(36)
        .nodePadding(40)
        .size([data.order_all.length * link_length + 110, height_s]);
    const sankey3 = d3_sankey_1.sankey()
        .nodeWidth(36)
        .nodePadding(40)
        .size([data.order_all.length * link_length + 110, height_s]);
    const path1 = function () {
        let curvature = 0.5;
        function link(d, i) {
            const x0 = pop_place(find_link(i).source).r_x, x1 = pop_place(find_link(i).target).r_x, xi = d3.interpolateNumber(x0, x1), x2 = xi(curvature), x3 = xi(1 - curvature), y0 = pop_place(find_link(i).source).r_y, y1 = pop_place(find_link(i).target).r_y;
            return ("M" +
                x0 +
                "," +
                y0 +
    ...
    const path2 = function () {
        let curvature = 0.5;
        function link(d, i) {
            const x0 = pop_place2(find_link1(i).source).r_x, x1 = pop_place2(find_link1(i).target).r_x, xi = d3.interpolateNumber(x0, x1), x2 = xi(curvature), x3 = xi(1 - curvature), y0 = pop_place2(find_link1(i).source).r_y, 
    ...
    const path3 = function () {
        let curvature = 0.5;
        function link(d, i) {
            const x0 = pop_place3(find_link2(i).source).r_x, x1 = pop_place3(find_link2(i).target).r_x, xi = d3.interpolateNumber(x0, x1), x2 = xi(curvature), x3 = xi(1 - curvature), y0 = pop_place3(find_link2(i).source).r_y, 
    ...
    const link1 = link1_data
        ...
        return Math.max(1, circle_r * 2 * 0.8
    ...
    force.on("tick", function () {
        ...
        d3.selectAll("circle")
            ...
            return pop_place(i).r_x;
        })
            .attr("cy", function (d, i) {
            return pop_place(i).r_y;
        });
        d3.selectAll(".node_text")
            ...
            .attr("y", function (d, i) {
            return pop_place(i).r_y - 30;
        });
        d3.selectAll(".info_rect")
            .attr("x", function (d, i) {
            return pop_place(i).r_x - 105;
        })
            .attr("y", function (d, i) {
            return pop_place(i).r_y - 120;
        });
        d3.selectAll(".info_text1")
            .style("text-anchor", "middle")
            .attr("x", function (d, i) {
            return pop_place(i).r_x;
        })
            .attr("y", function (d, i) {
            return pop_place(i).r_y - 100;
        });
        d3.selectAll(".info_text2")
            .style("text-anchor", "middle")
            .attr("x", function (d, i) {
            return pop_place(i).r_x;
        })
            .attr("y", function (d, i) {
            return pop_place(i).r_y - 80;
        });
        d3.selectAll(".info_text3")
            .style("text-anchor", "middle")
            .attr("x", function (d, i) {
            return pop_place(i).r_x;
        })
            .attr("y", function (d, i) {
            return pop_place(i).r_y - 60;
        });
        d3.selectAll(".link1_text")
            .attr("x", function (d, i) {
            return (pop_place2(i).r_x + pop_place2(i + 1).r_x) / 2;
        })
            .attr("y", function (d, i) {
            return (pop_place2(i).r_y + pop_place2(i + 1).r_y) / 2;
        });
        d3.selectAll(".link2_text")
            .attr("x", function (d, i) {
            return (pop_place3(i).r_x + pop_place3(i + 1).r_x) / 2;
        })
            .attr("y", function (d, i) {
            return (pop_place3(i).r_y + pop_place3(i + 1).r_y) / 2;
        });
    });
    //---Insert-------
    svg
        ...
        .style("stroke", "black");
    function push_place(parallel, j) {
        ...
        if (parallel % 2 == 0) {
            if (j < parallel / 2)
                return c_line + 75 + 150 * (j - parallel / 2);
            else
                return c_line + 75 - 150 * (parallel / 2 - j);
        }
        else {
            parallel = parallel - 1;
            if (j < parallel / 2)
    ...
    function find_link1(id) {
        return a_line2[id];
    }
    function pop_place3(id) {
        return place3[id];
    }
    function find_link2(id) {
        return a_line3[id];
    }
    ...
    risk1
        ...
        .text("P値:" + Statistics[0].IRP + "(" + Statistics[1].IRSD + ")");
    const rect2 = svg
        .append("rect")
        .attr("class", "rect2")
        .style("visibility", "hidden")
    ...
    risk2
        ...
        .text("P値:" + Statistics[0].SRP + "(" + Statistics[1].SRSD + ")");
    const rect3 = svg
        .append("rect")
        .attr("class", "rect3")
        .style("visibility", "hidden")
    ...
    cost1
        ...
        .text("P値:" + Statistics[0].LOSP + "(" + Statistics[1].LOSSD + ")");
    const rect4 = svg
        .append("rect")
        .attr("class", "rect4")
        .style("visibility", "hidden")
    ...
    cost2
        ...
        .text("P値:" + Statistics[0].COSTP + "(" + Statistics[1].COSTSD + ")");
});
