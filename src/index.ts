  ...
  if (order.isGroup) {
    ...
    place.push({
      ...
      r_y: push_place(
        ...
        seq_index,
        order.pre_item_id,
        data.groups[order.order_type as groupsKeys].length
      ),
    });
    data.groups[order.order_type as groupsKeys].forEach((_order, index) => {
      // 各オーダーが通る座標
      graph.orders.push({
        ..._order,
        id: order.id,
        ...
        r_y:
          ...
          (60 * (data.groups[order.order_type as groupsKeys].length - 1)) / 2 +
          60 * index,
      });
    });
    // 普通のオーダーの場合
...
if (PatientData) {
  while (data.order_all[i] && PatientData[j]) {
    if (data.order_all[i].isGroup) {
      const groupOrders =
        data.groups[data.order_all[i].order_type as groupsKeys];
    }
...
function getAllPathName(
  ...
  return path_id_array.map((id) => {
    ...
    if (order.isGroup || order.isInjection) {
      const group_orders = data.groups[order.order_type as groupsKeys];
...
let link2_data =
  svg2 && svg2.append("g").selectAll(".link2").data(graph2.links).enter();
...
function push_place(
  ...
  if (parallel % 2 == 0) {
    ...
    else return c_line + 75 - 150 * (parallel / 2 - j) + (group_num ?? 0) * 15;
  } else {
    parallel = parallel - 1;
...
const onClick = () => {
  if (document) {
    const form: HTMLInputElement = <HTMLInputElement>(
